...
- **`orders`** - Main order tracking table
  - Fields: `client_id`, `order_number`, `status`, `total_amount`, `company_id`
  - JSONB: `shipping_address`, `billing_address`
  - References: `clients`, `companies`

- **`order_items`** - Individual order line items
  - Fields: `product_id`, `product_type`, `quantity`, `price`
  - JSONB: `specifications`
  - References: `orders(id)`

- **`punchout_orders`** - RealPage/Entrada punch-out orders
  - Fields: `order_id`, `session_id`, `buyer_cookie`, `return_url`, `status`
  - JSONB: `customer_info`, `items`
  - References: `punchout_sessions(id)`

#### Clients & Companies
- **`clients`** - User/client accounts
  - Referenced by: `orders`, `companies`, `properties`

- **`companies`** - Enterprise client companies
  - Fields: `name`, `address`, `city`, `state`, `zip_code`, `phone`, `email`, `tax_id`
  - References: `punchout_sessions`, `orders`

#### Property Management
- **`properties`** - Client properties
  - References: `clients(id)`, `company_id`
  - Referenced by: `companies` RLS policies

- **`floor_plan_measurements`** - Window measurements
  - Fields: `width_whole`, `width_fraction`, `height_whole`, `height_fraction`
  - Fields: `blind_type`, `material_type`, `mount_type`, `product_id`
  - References: `products(id)`

#### Products & Pricing
- **`products`** - Product catalog
  - Referenced by: `floor_plan_measurements`, `order_items`

- **`sku_mappings`** - SKU to specification mapping
  - Fields: `sku`, `product_type`, `color`, `material`, `finish`, `series`, `opacity`
  - Product types: faux_wood, wood, roller_shade, vertical_blind, mini_blind

#### RealPage Integration
- **`punchout_sessions`** - RealPage/Entrada punch-out sessions
  - Fields: `buyer_cookie`, `return_url`, `user_id`, `company_id`, `status`
  - Status: active, completed, expired
  - Expires: 24 hours default
  - Has cleanup cron job

- **`realpage_invoices`** - RealPage invoices sent
  - Fields: `invoice_number`, `order_number`, `order_id`, `status`
  - JSONB: `invoice_data`, `realpage_response`
  - Timestamps: `sent_at`, `paid_at`
  - References: `orders(id)`

#### Shopping Cart
- **`carts`** - Shopping cart sessions
  - RLS: Allow all operations (anonymous support)
...
- **`chat_sessions`** - Chat session tracking
  - Fields: `last_viewed_at` (admin tracking)

- **`chat_messages`** - Chat messages
  - Fields: `admin_viewed` (boolean for unread tracking)
  - Fields: `sender_type` (customer/admin)

#### Sample Requests
- **`sample_requests`** - Free sample orders
  - Referenced in migrations

#### Pricing Tables
- **`faux_wood_premium_pricing`** - Faux wood premium pricing
- **`mini_blind_vinyl_pricing`** - Mini blind vinyl pricing
- **`mini_blind_aluminum_pricing`** - Mini blind aluminum pricing
- **`wood_blind_pricing`** - Wood blind pricing (from migrations)
- **`roller_shade_pricing`** - Roller shade pricing (from migrations)

#### Operations & Inventory
- **`work_orders`** - Work order management
- **`cutting_orders`** - CNC cutting orders
- **`inventory`** - Inventory management
- **`delivery_routes`** - Delivery routing
- **`measurement_appointments`** - Measurement scheduling

#### Accounting
- **`invoices`** - Invoice records
- **`payments`** - Payment records
- **`accounts_receivable`** - AR tracking
- **`financial_reports`** - Financial report history

#### Marketing & SEO
- **`google_merchant_products`** - Google Shopping products
- **`google_merchant_feed_history`** - Feed generation history
- **`feed_regeneration_queue`** - Feed regeneration queue
- **`competitor_price_tracking`** - Competitor price tracking
- **`marketing_tables`** - Marketing campaign data
...
#### Testing
- **`load_test_results`** - Load test results storage

### Key Relationships

#### Order Flow
```
clients → companies → punchout_sessions → punchout_orders → orders → order_items
                                                              ↓
                                                   realpage_invoices
```
...
#### Property Management
```
clients → properties → rooms → windows → floor_plan_measurements → products
```

#### Operations Flow
```
orders → work_orders → cutting_orders → inventory
```
...
**⚠️ Important:** Local migrations may not match production database. See `docs/database/MIGRATION_WORKFLOW.md` for migration workflow.
